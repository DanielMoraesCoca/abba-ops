# Setor Capacitação

**Missão:** mudar como as pessoas do cliente trabalham — plataforma, conteúdo e presencial ([estágio 08](../02-jornada-do-cliente/08-capacitacao-e-transformacao.md)).
**Dono:** Pedro · suplente: Daniel (matriz no [README](README.md))

## Responsabilidades

- Plano de capacitação por cliente (do [template](../04-entrega/plano-de-capacitacao.md)): trilhas, cadência, sessões presenciais
- Conteúdo: os materiais finais da Academy estão prontos ([catálogo](../08-materiais/README.md)) — prioridade atual: **gravar os 3 vídeos de maior alavancagem (1.3.3, 1.3.1, 2.1.2), imprimir os 6 cards do Kickoff e carregar as trilhas no portal**; personalização [PERSONALIZAR] com dados do 1º cliente
- Sessões presenciais: preparar, conduzir, colher presença e feedback
- Progressão: acompanhar adoção por cliente (níveis, desafios, Bússola) e agir sobre departamentos travados
- Graduação: certificados, campeões formados, cerimônia de encerramento

## Checklist semanal

- [ ] Adoção por cliente ativo: % participantes ativos na semana? Departamento travado?
- [ ] Próxima sessão presencial: data, pauta e material prontos?
- [ ] 1 avanço de conteúdo na semana (vídeo gravado, lição revisada)? Qual o próximo?
- [ ] Feedback das sessões/lições registrado e triado?

## Ferramentas
Portal ABBA (trilhas, desafios, Bússola, Iris, admin) · Drive `02 Clientes/<Nome>/05 Capacitacao/`

## Métricas
% participantes ativos/semana · distribuição por nível · desafios concluídos · entradas de Bússola/participante · presença nas sessões presenciais
