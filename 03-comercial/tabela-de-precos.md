# Tabela de Preços — ABBA (v1 · 2026-07-23)

> **Tabela vigente**, calculada no cenário C da [proposta do especialista](precificacao-proposta-especialista.md): renda-alvo R$ 20 mil/sócio/mês, 35h/semana cada, utilização 55% → **piso R$ 331/h**; hora-alvo de posicionamento **~R$ 400/h** (banda alta do mercado sênior R$ 200–450/h, justificada por método proprietário + ferramentas próprias + entrega de sistemas). Provisão tributária: 12% (Simples Anexo III via Fator R). Alterações → [registro de decisões](../05-interno/registro-de-decisoes.md).

## Tabela vigente (v1)

| Produto | Preço-alvo | Faixa (por porte/escopo) | Pagamento |
|---|---|---|---|
| Workshop de Descoberta de Shadow AI | **R$ 14.000** | R$ 12–16 mil | 50/50 |
| Avaliação de Prontidão para IA | **R$ 28.000** | R$ 24–35 mil | 50/50 |
| Sprint LGPD + Governança de IA | **R$ 24.000** | R$ 20–32 mil | 50/50 |
| **Programa completo de transformação (16 sem)** | **R$ 185.000** | R$ 165–240 mil | 30/30/40 por marco |
| Manutenção mensal (agentes + plataforma + ritual) | **R$ 9.500/mês** | R$ 8–13 mil/mês | mensal; anual −8% |
| Mini-ciclo (caso de uso adicional) | **R$ 42.000** | R$ 35–60 mil | 50/50 |

**Cheques de sanidade:** hora implícita de cada pacote entre R$ 357–475 ✓ dentro/acima da banda-alvo · programa a R$ 185K = 1,24× o custo no piso C (colchão) · charter máximo −20% → programa charter R$ 148K, ainda ≈ piso ✓.

Regras comerciais:
- Produto de entrada credita 100% no programa em até 90 dias (exceção: Sprint LGPD não credita)
- Desconto **só** via charter com contrapartidas (Anexo III do [contrato](contrato-sow-esqueleto.md)), máx. 20%
- Abaixo do piso do pacote = não vender · reajuste anual IPCA nos recorrentes
- Disciplina de descoberta: registrar TODA reação a preço na [planilha](precificacao-planilha.md) — 2 sustos = revisar comunicação de valor; 2 "razoável" fácil = subir 15% na próxima

## Rascunho v2 — NÃO vigente (ativar após as 3 primeiras reações reais de preço)

> Decisão dos sócios (2026-07-25, análise estratégica E1/E2): os produtos abaixo já existem como oferta desenhada, mas seus preços só entram em vigor na v2 — depois de registradas as reações das 3 primeiras conversas reais com a v1. Até lá: a manutenção vigente é o preço único de R$ 9.500/mês da tabela v1, e o Conselheiro de IA só é vendido se um prospect pedir (usar a faixa rascunho com transparência de "condição de lançamento").

| Produto (v2) | Preço-alvo | Racional |
|---|---|---|
| [Conselheiro de IA ABBA](conselheiro-de-ia.md) — Trimestral | R$ 6.000/mês | Senioridade recorrente, custo de entrega baixo (CAIO fracionário) |
| [Conselheiro de IA ABBA](conselheiro-de-ia.md) — Mensal | R$ 9.000/mês | Presença mensal + arbitragens ilimitadas |
| Manutenção — Camada 1 **Operação** | R$ 6.000/mês | Monitoramento + SLA + relatório mensal (sem horas de evolução) |
| Manutenção — Camada 2 **Evolução** | R$ 9.500/mês | = a manutenção v1 atual (âncora do meio — quem compara escolhe camada, não preço) |
| Manutenção — Camada 3 **Estratégia** | R$ 15.000–18.000/mês | Evolução + Conselheiro de IA trimestral embutido |

Detalhe das camadas no [SLA](../04-entrega/sla-manutencao.md); produto do Conselheiro em [conselheiro-de-ia.md](conselheiro-de-ia.md).

## Histórico de versões

| Versão | Data | Mudança |
|---|---|---|
| v1 | 2026-07-23 | Primeira tabela vigente — cenário C, hora-alvo R$ 400, pesquisa tributária e de mercado citada na proposta do especialista |

---

## Anexo — referência histórica (NÃO USAR)

> ⚠️ Preços da fase anterior da estratégia (2026-04), **descartados por decisão dos sócios em 2026-07-22**. Mantidos apenas como registro do posicionamento anterior:
>
> - Programa de Instalação de Capacidade ("Capability Installation Program", 29 semanas): R$ 280–450 mil, incluindo 12 meses de Sustain
> - Workshop Shadow AI Discovery: R$ 8–15 mil
> - AI Readiness Assessment standalone: R$ 15–25 mil
> - Sprint LGPD + Governança: R$ 20–35 mil
> - Assinatura Sustain (ano 2+): R$ 60–120 mil/ano
> - Mini-ciclo por caso de uso: R$ 30–80 mil
>
> Fonte: `abba-portal/docs/strategic/PRICING_SHEET_SOURCE.md` (marcado como histórico).
