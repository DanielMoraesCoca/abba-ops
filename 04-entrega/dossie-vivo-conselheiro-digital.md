...
> **Camada:** processo de entrega. **Status: CONSTRUÍDO (2026-07-30), ativação gateada no 1º cliente em manutenção.** O código vive no assessment-brain (`src/brain/`); arquitetura em [`../05-interno/arquitetura-cerebro-conselheiro.md`](../05-interno/arquitetura-cerebro-conselheiro.md); plano e fases em [`../05-interno/plano-implementacao-conselheiro.md`](../05-interno/plano-implementacao-conselheiro.md). Este doc é o manual de operação para o Conselheiro humano. Nome comercial do produto = decisão de sócios (Fase 4); até lá, internamente: **Dossiê Vivo**.
...
2. Consultas na preparação: `abba brain facts <eng>` (verdades vigentes, com validade) · `--as-of 2026-03-01` (o que valia então) · `abba brain profile <eng>` (dossiê compilado) · `abba brain health <eng>` (7 componentes, 0–100 — abaixo de 70, olhar o detalhe antes do ritual).
3. Identidade por cliente: `abba learn soul <cliente> --set '<json>'` (valores/voz/limites que moldam a narrativa do brief; sanitizado anti-injeção).
...
1. Dados reais entrando (a disciplina de ingestão desde o kickoff) · 2. Calibração do golden set (20–50 saídas notadas pelos sócios) · 3. Cron do sono (1 linha) · 4. Decisões de sócios pendentes: nome comercial, melhorias de memória priorizadas ([estudo](../05-interno/estudo-memoria-agentica.md) — nº 1: autoridade de origem anti-poisoning ANTES do 1º cliente) e se/como o cérebro entra no discurso comercial (hoje: diferencial interno da cadeira de Conselheiro, nunca prometido como produto avulso).
