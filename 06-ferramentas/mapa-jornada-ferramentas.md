...
| 02 Degustação | **assessment web** (`assessment.abbaservices.com.br`, no ar: aposta 2 ✅ realizada) + assessment-brain (`abba scout`) | Site vivo (ainda não aberto ao público: R4/R16); CLI funcional | O documento abre com o [Mapa de Vazamento](../03-comercial/mapa-de-vazamento.md) (faixa em R$, 48h); apresentação sempre ao vivo; exige provedor de busca real configurado ({{PEDRO VERIFICAR}}) |
| 03–04 Proposta e contrato | Templates deste repo + Drive + assinatura eletrônica | Templates prontos | Advogado pendente (P4) · plataforma de assinatura a escolher (tabela de preços v1 travada desde 2026-07-23) |
| 05 Onboarding | Portal (tenant, contas) + assessment-brain (engajamento) | Funcional | Sessão **assinada e fail-closed** desde 24/08 (JWT; sem segredo, o sign-in de produção recusa): o que falta é SSO corporativo: **não prometer requisitos de procurement enterprise** até o cutover |
| 06 Avaliação profunda | assessment-brain (pipeline 25 dimensões, relatórios, one-pager, share-links) | Funcional; 429 testes verdes | **Validado apenas em dados sintéticos**: o 1º cliente real é também a 1ª validação real (mitigação: escopo charter apertado); certificação com modelo de nível cliente pendente |
...
| 08 Capacitação | Portal (as 27 aulas, desafios, Bússola, Iris, admin) + presencial | Conteúdo FINAL; 27 aulas NO portal | Materiais completos ([catálogo](../08-materiais/README.md)): roteiros, kit de Kickoff, cards, prompts, desafios, lições CrewAI · as 27 aulas já estão carregadas no portal (o preflight audita) · falta: gravar vídeos, imprimir cards · **é UMA formação só**: a escada de níveis foi revogada em 26/08 e as trilhas por papel em 02/09 |
...
| Transversal | **Conselheiro Digital / Dossiê Vivo** (`src/brain/` no assessment-brain): memória bitemporal por cliente, ciclo noturno com teto de gasto, diário de decisões→resultados, auditoria da própria memória, playbooks e benchmark de tenure | **Construído (Fases 0–2 + Ondas 1–3), suíte 429/429** | **Não ativado**: falta validação com LLM real, golden set calibrado e cron ligado: ver [runbook de ativação](runbook-ativacao.md). Operação diária no [dossiê vivo](../04-entrega/dossie-vivo-conselheiro-digital.md) |
